// User-level non_maskable interrupt handler support.
// Rather than register the C non_maskable interrupt handler directly with the
...
//
// Set the non_maskable interrupt handler function.
// If there isn't one yet, _nmskint_handler will be 0.
// The first time we register a handler, we need to
// allocate an exception stack (one page of memory with its top
// at UXSTACKTOP), and tell the kernel to call the assembly-language
// _nmskint_upcall routine when a non_maskable interrupt occurs.
//
void
