// User-level SIMD floating point error handler support.
// Rather than register the C SIMD floating point error handler directly with the
...
//
// Set the SIMD floating point error handler function.
// If there isn't one yet, _SIMDfperror_handler will be 0.
// The first time we register a handler, we need to
// allocate an exception stack (one page of memory with its top
// at UXSTACKTOP), and tell the kernel to call the assembly-language
// _SIMDfperror_upcall routine when a SIMD floating point error occurs.
//
void
