// User-level breakpoint handler support.
// Rather than register the C breakpoint handler directly with the
...
//
// Set the breakpoint handler function.
// If there isn't one yet, _bpoint_handler will be 0.
// The first time we register a handler, we need to
// allocate an exception stack (one page of memory with its top
// at UXSTACKTOP), and tell the kernel to call the assembly-language
// _bpoint_upcall routine when a breakpoint occurs.
//
void
