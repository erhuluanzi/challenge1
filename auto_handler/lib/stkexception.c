// User-level stkexception handler support.
// Rather than register the C stkexception handler directly with the
...
//
// Set the stkexception handler function.
// If there isn't one yet, _stkexception_handler will be 0.
// The first time we register a handler, we need to
// allocate an exception stack (one page of memory with its top
// at UXSTACKTOP), and tell the kernel to call the assembly-language
// _stkexception_upcall routine when a stkexception occurs.
//
void
